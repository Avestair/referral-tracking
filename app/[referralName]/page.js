...
export default async function TheaterLandingPage({ params }) {
  ...
  const { referralName } = await params;
  ...
  try {
    ...
    // Log request start
    // console.log(
    //   `[Visitor Tracking] Starting tracking for IP: ${ip} (Referral ID: ${referral.id})`
    // );
    ...
    if (!existingVisit?.length) {
      ...
      //   console.log(
      //     `[Visitor Tracking] Successfully tracked new visitor (IP: ${ip}, Referral ID: ${referral.id})`
      //   );
